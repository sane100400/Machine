...
```bash
# 프레임워크별 취약점 우선순위
python3 $MACHINE_ROOT/tools/decision_tree.py vuln-priority --framework <flask|django|express|php|spring|go|rails>

# 실패 시 다음 행동 조회
python3 $MACHINE_ROOT/tools/decision_tree.py next --agent web --trigger <trigger_name>
# Triggers: no_vuln_found, ambiguous_vuln

python3 $MACHINE_ROOT/tools/decision_tree.py record --agent web --trigger <trigger> --action-id <id>
```

Confidence scoring: state.py에 직접 기록
```bash
python3 $MACHINE_ROOT/tools/state.py set --key vuln_confidence --val "0.9" --src web_analysis.md --agent web
```
