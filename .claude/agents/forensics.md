...
```bash
python3 $MACHINE_ROOT/tools/decision_tree.py next --agent forensics --trigger <trigger_name>
# Triggers: unknown_filetype, stego_deadend, memory_failure, pcap_deadend

python3 $MACHINE_ROOT/tools/decision_tree.py record --agent forensics --trigger <trigger> --action-id <id>
```

**Layer depth guard**: 추출 레이어 5단계 초과 금지. 같은 파일 타입 3회 반복 시 즉시 중단.
