...
# Solve a CTF challenge (fully autonomous — solves + writes up + indexes)
./machine.sh ctf ./challenge.zip

# Specify category + remote server
./machine.sh ctf ./challenge.zip pwn host1.dreamhack.games:12345

# Add a challenge.md for flag format / description constraints
echo 'Flag format: `DH{[a-zA-Z0-9_]+}`' > ./challenge_dir/challenge.md
./machine.sh ctf ./challenge_dir crypto

# Import existing writeups / reindex knowledge DB
./machine.sh learn --import writeup.md
./machine.sh learn --reindex
...
| **Challenge metadata** | Auto-reads `challenge.md` for flag format regex, description, constraints |
| **Failure decision trees** | `decision_tree.py` — agents call on failure to get next action (not inline text) |
...
└── challenges/              # Solve records (gitignored)
    ├── _template.md
    └── <challenge>.md       # Auto-generated after every solve
```

### Knowledge Accumulation Flow

```
Solve challenge (ctf mode)
        ↓
Auto-generate writeup (every solve)
        ↓
...
|--------|------|-------------|
| ExploitDB | `~/datasources/exploitdb/` | Public exploit CSV database |
| Nuclei | `~/datasources/nuclei-templates/` | Vulnerability scan templates |
| PoC-in-GitHub | `~/datasources/PoC-in-GitHub/` | CVE-linked PoC collection |
| PayloadsAllTheThings | `~/references/PayloadsAllTheThings/` | Payloads & technique docs |
...
```
Machine/
├── machine.sh                       # Autonomous launcher (ctf / status / logs / learn --import)
...
│   ├── decision_tree.py             # Failure handling engine (replaces inline trees)
