...
| 카테고리 | 파이프라인 | 모델 |
|---------|-----------|------|
| **PWN** | pwn → critic → verifier | opus |
| **REV** | rev → critic → verifier | opus |
| **WEB** | web → critic → verifier | opus |
| **CRYPTO** | crypto → critic → verifier | opus |
| **FORENSICS** | forensics → critic → verifier | opus |
| **WEB3** | web3 → critic → verifier | opus |
...
├── .claude/
│   ├── agents/                      # 에이전트 정의
│   │   ├── pwn.md                   # PWN: Ghidra + GDB + pwntools + ROPgadget
│   │   ├── rev.md                   # REV: Ghidra + GDB + Frida + z3 + angr
│   │   ├── web.md                   # WEB: ffuf + sqlmap + dalfox + Playwright
│   │   ├── crypto.md                # CRYPTO: SageMath + z3 + RsaCtfTool + hashcat
│   │   ├── forensics.md             # FORENSICS: binwalk + volatility3 + tshark + zsteg
│   │   ├── web3.md                  # WEB3: Slither + Mythril + Foundry + cast
│   │   ├── critic.md
│   │   ├── verifier.md
│   │   ├── reporter.md
│   │   └── _reference/
